/**
 * PixiBoard.tsx - Versão ESTÁVEL e FUNCIONAL (PixiJS v8)
 * Foco total em Explosões, Peças e Performance.
 */
...
declare global { namespace JSX { interface IntrinsicElements { pixiContainer: any; pixiGraphics: any; pixiText: any; } } }
...
const PixiBoard: React.FC<PixiBoardProps> = ({ board, containerWidth, hoverCell, draggedPiece, selectedPiece, clearingCells, floatingPoints, hintPosition, theme, onCellClick }) => {
  const cellSize = Math.floor(containerWidth / B_SIZE);
  const width = cellSize * B_SIZE;
  const height = cellSize * B_SIZE;

  const appRef = useRef<any>(null);
  const vfxLayerRef = useRef<any>(null);

  const activePiece = draggedPiece?.piece ?? selectedPiece?.piece ?? null;

  const specialtyStyle = useMemo(() => new TextStyle({ fontFamily: 'system-ui', fontSize: Math.floor(cellSize * 0.5), fontWeight: 'bold', fill: '#ffffff', dropShadow: { color: '#000000', alpha: 0.5, blur: 5, distance: 2 } }), [cellSize]);
  const floatPointStyle = useMemo(() => new TextStyle({ fontFamily: 'system-ui', fontSize: Math.floor(cellSize * 0.55), fontWeight: '950', fill: '#ffffff', dropShadow: { color: '#000000', alpha: 0.7, blur: 10, distance: 4 } }), [cellSize]);
  ...
  const triggerExplosion = useCallback((group: { r: number; c: number }[], colorHex: number) => {
    ...
    if (!app || !vfx || group.length === 0) return;
    const cx = (group[0].c + 0.5) * cellSize, cy = (group[0].r + 0.5) * cellSize;

    for (let i = 0; i < 30; i++) {
        const p = new Graphics();
        p.circle(0, 0, 3 + Math.random() * 5).fill({ color: colorHex });
        p.x = cx; p.y = cy;
        const angle = Math.random() * Math.PI * 2, speed = 150 + Math.random() * 350;
        const vx = Math.cos(angle) * speed;
        let vy = Math.sin(angle) * speed - 180;
        let life = 1.0;
        vfx.addChild(p);
        ...
        const update = (time: any) => {
            ...
            p.x += vx * dt; p.y += vy * dt;
            vy += 900 * dt; life -= 2.6 * dt;
            p.alpha = life; p.scale.set(life);
            if (life <= 0) { app.ticker.remove(update); if (p.parent) p.parent.removeChild(p); }
        };
  ...
  useEffect(() => {
    (window as any).triggerPixiExplosion = (group: any[], color: Color) => triggerExplosion(group, colorToHex[color] || 0xffffff);
  ...
  const drawBoard = useCallback((g: Graphics) => {
    ...
    for (let r = 0; r < B_SIZE; r++) {
      for (let c = 0; c < B_SIZE; c++) {
        const x = c * cellSize, y = r * cellSize, cell = board[r]?.[c];
        if (cell) {
          ...
          if (clearingCells.has(`${r}-${c}`)) g.roundRect(x + 2, y + 2, cellSize - 4, cellSize - 4, 12).stroke({ color: 0xffffff, width: 4.5, alpha: 0.9 });
  ...
  return (
    <Application ref={appRef} width={width} height={height} backgroundAlpha={0} antialias={true} resolution={1}>
      <pixiContainer {...({ isRenderGroup: true } as any)}>
        <pixiGraphics draw={drawBoard} />
        {ghostData && <pixiGraphics draw={(g: Graphics) => {
            g.clear();
        ...
        {floatingPoints.map((pt) => <pixiText key={pt.id} text={`+${pt.points}`} x={(pt.c + 0.5) * cellSize} y={(pt.r + 0.5) * cellSize - 25} anchor={0.5} style={floatPointStyle} />)}
        {hintPosition && <pixiGraphics draw={(g: Graphics) => g.clear().rect(hintPosition.c * cellSize + 4, hintPosition.r * cellSize + 4, cellSize - 8, cellSize - 8).stroke({ color: 0xfff700, width: 4, alpha: 0.85 })} />}
      </pixiContainer>
      <pixiContainer ref={vfxLayerRef} />
      <pixiContainer>
